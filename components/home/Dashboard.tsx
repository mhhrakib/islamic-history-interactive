...
const DashboardTopicCard: React.FC<{
  ...
  >
    <p className="font-semibold text-sm text-secondary dark:text-red-400">
      {topic.period}
    </p>
    <h4 className="font-serif font-semibold text-primary dark:text-green-300 mt-1">
      {topic.name}
    </h4>
...
const MobileStatCard: React.FC<{
  ...
  <div className="bg-surface dark:bg-gray-800 rounded-lg shadow-md p-3 flex flex-col items-center text-center h-full justify-center">
    <div className="text-secondary dark:text-red-400 mb-2">{icon}</div>
    <p className="text-2xl font-bold text-primary dark:text-gray-100">
      {value}
    </p>
    <p className="text-xs text-text-secondary dark:text-gray-400 font-semibold uppercase tracking-wider mt-1">
      {title}
    </p>
  </div>
);
...
export const Dashboard: React.FC<DashboardProps> = ({
  ...
  const featuredTopics = useMemo(
    () => allTopics.filter((t) => t.isFeatured),
    [allTopics]
  );
  ...
  const frequentTopics = useMemo(() => {
    return allTopics
      .map((topic) => ({
        ...topic,
        completedCount: topic.events.filter((event) =>
          profile.completedEventIds.includes(event.id)
        ).length,
      }))
  ...
  const lastViewedTopic = profile?.lastViewedTopicId
    ? allTopics.find((t) => t.id === profile.lastViewedTopicId)
    : null;
  const lastViewedEvent =
    lastViewedTopic && profile?.lastViewedEventId
      ? lastViewedTopic.events.find((e) => e.id === profile.lastViewedEventId)
      : null;
  ...
  return (
    <div className="space-y-12 animate-fade-in">
      <section className="md:hidden">
        <h2 className="text-2xl font-serif font-bold text-primary dark:text-green-300 text-center mb-4">
          {t("home.yourStats")}
        </h2>
        <div className="grid grid-cols-3 gap-4">
          ...
          />
          <MobileStatCard
            icon={<StreakIcon className="w-7 h-7" />}
      ...
      {lastViewedTopic && (
        <section>
          <div className="max-w-4xl mx-auto bg-surface dark:bg-gray-800 p-6 rounded-xl shadow-lg border border-primary/20 dark:border-green-300/20 text-center">
            <h2 className="text-2xl font-serif font-bold text-primary dark:text-green-300 mb-2">
            ...
            <p className="text-xl font-semibold font-serif text-secondary dark:text-red-400 mb-4">
              {lastViewedEvent
                ? `"${lastViewedEvent.title}"`
                : lastViewedTopic.name}
            </p>
            <button
              onClick={() =>
                onSelectTopic(lastViewedTopic.id, lastViewedEvent?.id)
              }
              className="px-6 py-2 bg-primary dark:bg-green-600 text-white rounded-lg font-semibold hover:bg-opacity-90 transition-all shadow-sm"
            >
              {t("home.jumpBackIn")}
            </button>
          </div>
