...
import { KaabaIcon, ArrowLeftIcon } from './icons';
      ...
      <div className="container mx-auto px-4 md:px-8">
        <div className="py-3 flex items-center gap-4">
          ...
          <div className="flex items-center gap-4 flex-grow">
            ...
            <div>
              <h1 className="text-xl md:text-2xl font-serif font-semibold text-primary dark:text-green-300">{t('titleBar.title')}</h1>
              <p className="text-sm text-text-secondary dark:text-gray-400 font-sans hidden sm:block">{t('titleBar.subtitle')}</p>
            </div>
          </div>
        ...
        <div 
          className={`
            ...
            ${isStickyHeaderVisible ? 'max-h-20 py-2 opacity-100 border-t border-border-color/50 dark:border-gray-700/50' : 'max-h-0 py-0 opacity-0'}
          `}
          aria-hidden={!isStickyHeaderVisible}
        >
           <p className="font-sans font-bold text-primary dark:text-green-300 truncate">{stickyHeaderText}</p>
        </div>
      </div>
    </header>
  );
