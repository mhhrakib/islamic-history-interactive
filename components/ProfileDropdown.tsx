...
export const ProfileDropdown: React.FC<ProfileDropdownProps> = ({ onLoginClick, onNavigate }) => {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button onClick={() => setIsOpen(!isOpen)} className={`w-10 h-10 rounded-full bg-border-color dark:bg-gray-700 overflow-hidden ring-2 ring-offset-2 ring-offset-surface dark:ring-offset-gray-800 ${isAdmin ? 'ring-secondary dark:ring-red-500' : 'ring-primary dark:ring-green-400'}`}>
      ...
      {isOpen && (
        <div className="absolute top-full right-0 mt-2 w-56 bg-surface dark:bg-gray-800 rounded-lg shadow-xl border border-border-color dark:border-gray-700 z-50 animate-dropdown">
          <div className="p-2 border-b border-border-color dark:border-gray-700">
            <p className="font-semibold text-text-primary dark:text-gray-100 truncate px-2">{user.name}</p>
            <p className="text-xs text-text-secondary dark:text-gray-400 px-2 capitalize flex items-center gap-1.5">
              {isAdmin && <span className="text-xs font-bold text-secondary dark:text-red-400">[ADMIN]</span>}
          ...
          <div className="p-1">
            {onNavigate && (
                ...
                >
                  ...
                  {isAdmin ? t('profileDropdown.adminConsole') : t('profileDropdown.myProfile')}
                </button>
            )}
            <button
