...
import { EditIcon, TrashIcon, CheckCircleIcon } from "../common/icons";
...
export const TopicCard: React.FC<TopicCardProps> = (props) => {
  ...
  return (
    <div
      className={`bg-surface dark:bg-gray-800 rounded-xl shadow-md p-6 w-full h-full flex flex-col hover:shadow-lg transition-shadow duration-300 relative group ${
        draggable ? "dnd-item" : ""
      }`}
      draggable={draggable}
      {...dragProps}
      ...
      {isAdmin && (
        <div className="absolute top-3 right-3 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
          ...
          <button
            ...
            className="p-2 bg-red-100 text-red-700 rounded-full hover:bg-red-200 dark:bg-red-900/50 dark:text-red-300 dark:hover:bg-red-900/80"
          >
      ...
      >
        <div className="flex-grow">
          <div className="flex justify-between items-start">
            <p className="font-sans text-sm font-bold text-secondary dark:text-red-400">
              {topic.period}
            </p>
            {isCompleted && !isAdmin && (
              <CheckCircleIcon className="w-6 h-6 text-primary dark:text-green-300" />
            )}
          </div>
          <h3 className="font-serif text-2xl font-semibold text-primary dark:text-green-300 mt-1">
            {topic.name}
          </h3>
          <p className="font-sans text-base text-text-secondary dark:text-gray-400 mt-1">
            {topic.title}
          </p>
        ...
        <div className="mt-4 pt-4 border-t border-border-color/70 dark:border-gray-700/70 flex justify-between items-center">
          <p className="font-semibold text-primary dark:text-green-300">
            {t("topicCard.explore")}
          </p>
