...
export const LeaderboardPage: React.FC<LeaderboardPageProps> = ({
  ...
  const sortedLeaderboard = useMemo(() => {
    ...
    if (user && profile && profile.globalQuizStats.highScore > 0) {
      ...
      if (
        !userLeaderboardData.some((p) => p.name.includes(t("leaderboard.you")))
      ) {
          ...
          {sortedLeaderboard.map((player, index) => (
            <li
              ...
              className={`flex items-center gap-4 p-4 ${
                player.name.includes(t("leaderboard.you"))
                  ? "bg-green-50 dark:bg-green-900/40"
                  : ""
              }`}
            >
              <div
                className={`w-10 text-center font-bold text-xl ${getRankColor(
                  player.rank
                )}`}
              >
                {player.rank}
              ...
              <div className="flex-grow">
                ...
                <p className="text-sm text-text-secondary dark:text-gray-400">
                  {player.score.toLocaleString()} {t("leaderboard.points")}
                </p>
              </div>
              {player.rank <= 3 && (
                <TrophyIcon
      ...
      <main className="flex-grow container mx-auto p-4 md:p-8">
        ...
        <div className="mt-8 text-center">
          <button
            ...
            className="px-6 py-3 bg-secondary dark:bg-red-600 text-white rounded-lg font-semibold hover:bg-opacity-90"
          >
