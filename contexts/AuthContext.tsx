...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    try {
      ...
      if (storedUser) {
        ...
        if (parsedUser.provider !== 'guest' && parsedUser.provider !== 'admin') {
  ...
  const login = (provider: UserProvider) => {
    ...
    switch (provider) {
        case 'google':
            newUser = { id: 'user-google-123', name: 'Al-Khwarizmi', avatarUrl: 'https://i.pravatar.cc/150?u=khwarizmi', provider: 'google' };
            loadProfile(newUser.id);
            break;
        case 'facebook':
            newUser = { id: 'user-facebook-456', name: 'Ibn Battuta', avatarUrl: 'https://i.pravatar.cc/150?u=battuta', provider: 'facebook' };
            loadProfile(newUser.id);
            break;
        case 'admin':
            newUser = { id: 'user-admin-001', name: 'Site Admin', avatarUrl: 'https://i.pravatar.cc/150?u=admin', provider: 'admin' };
            setIsAdmin(true);
            break;
        case 'guest':
        default:
            newUser = { id: 'user-guest-789', name: 'Guest', avatarUrl: '', provider: 'guest' };
            break;
    }
    setUser(newUser);
  ...
  const updateProfile = (updatedProfile: UserProfile) => {
    ...
    localStorage.setItem(`profile-${updatedProfile.userId}`, JSON.stringify(updatedProfile));
  };
  
  const markEventAsCompleted = (eventId: number) => {
    if (profile && user?.provider !== 'guest' && !profile.completedEventIds.includes(eventId)) {
  ...
  return (
    <AuthContext.Provider value={{ user, profile, isAdmin, login, logout, markEventAsCompleted, updateQuizScore, setLastViewedLocation }}>
      {children}
