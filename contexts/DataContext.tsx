...
interface DataContextType {
    ...
    addTopic: (eraId: number, data: { name: string, title: string, period: string, bio: string, isFeatured?: boolean }) => void;
    ...
    reorderEvents: (topicId: number, startIndex: number, endIndex: number) => void;
...
export const DataProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
      ...
      const loadData = async () => {
        ...
        try {
          ...
          if (storedData) {
            ...
            const dataFile = language === 'bn' ? 'rawHistoricalData_bn.json' : 'rawHistoricalData.json';
    ...
    const updateEra = (updatedEra: HistoricalEra) => {
        setEras(prevEras => prevEras.map(era => era.id === updatedEra.id ? updatedEra : era));
    };

    const addTopic = (eraId: number, data: { name: string, title: string, period: string, bio: string, isFeatured?: boolean }) => {
        setEras(prevEras => {
            ...
            return prevEras.map(era => 
                era.id === eraId 
                ? { ...era, topics: [...era.topics, newTopic] }
                : era
            );
        });
    };
    ...
    const updateTopic = (updatedTopic: HistoricalTopic) => {
        setEras(prevEras => prevEras.map(era => ({
            ...era,
            topics: era.topics.map(topic => topic.id === updatedTopic.id ? updatedTopic : topic)
        })));
    };
    ...
    const deleteTopic = (topicId: number) => {
        ...
        })).filter(era => era.topics.length > 0 || eras.length > 1)); // Prevents deleting the last era if it becomes empty, unless it's the only one. A better UX would be to handle this in the UI.
    };
    
    const addEvent = (topicId: number, newEventData: Omit<AppEvent, 'id'>) => {
        setEras(prevEras => {
            const allEvents = prevEras.flatMap(e => e.topics).flatMap(t => t.events);
        ...
        setEras(prevEras => prevEras.map(era => ({
            ...
            topics: era.topics.map(topic => {
                if (topic.id === topicId) {
                    return {
                        ...topic,
                        events: topic.events.map(event => event.id === updatedEvent.id ? updatedEvent : event)
                    };
                }
                return topic;
            })
    ...
    const reorderTopics = (eraId: number, startIndex: number, endIndex: number) => {
        setEras(prevEras => prevEras.map(era => {
            if (era.id === eraId) {
    ...
    const reorderEvents = (topicId: number, startIndex: number, endIndex: number) => {
        setEras(prevEras => prevEras.map(era => ({
            ...era,
            topics: era.topics.map(topic => {
                if (topic.id === topicId) {
                    return { ...topic, events: reorder(topic.events, startIndex, endIndex) };
                }
                return topic;
            })
        })));
    ...
    const importData = (newEras: HistoricalEra[]) => {
        setEras(newEras);
        alert('Data successfully imported. The application will now use the new data set.');
    };

    return (
        <DataContext.Provider value={{ eras, isLoading, addEra, updateEra, addTopic, updateTopic, deleteTopic, addEvent, updateEvent, deleteEvent, reorderEras, reorderTopics, reorderEvents, importData }}>
            {children}
